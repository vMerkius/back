...
const sendAccountActivation = require('./../utils/emailsec');
...
const createSendToken = (user, statusCode, res) => {
  ...
  console.log('Cookie set with options:', cookieOptions, 'token:', token);
...
exports.checkToken = catchAsync(async (req, res, next) => {
  let token;
  if (req.cookies.jwt) {
    token = req.cookies.jwt; // Pobieranie tokenu z cookies
  } else if (
    req.headers.authorization &&
    req.headers.authorization.startsWith('Bearer')
  ) {
    token = req.headers.authorization.split(' ')[1]; // Pobieranie tokenu z nagłówka Authorization
  }
